...
| **Railway "Healthcheck failure"** (build & deploy succeed) | Almost always the app isn't serving, or the DB isn't linked. Checklist: (1) a **PostgreSQL service** exists in the project and the app's `DATABASE_URL` references it (`${{Postgres.DATABASE_URL}}`); (2) check **Deploy Logs** — `prisma migrate deploy` must succeed before the server starts; (3) `/api/health` now always returns **200** while the process is up (DB status is in the body), and the server keeps listening while it retries the DB, so a green healthcheck means the app is up even if data calls are still warming. If it still fails, the migrate step is likely erroring — read the deploy log for the Prisma error. |
